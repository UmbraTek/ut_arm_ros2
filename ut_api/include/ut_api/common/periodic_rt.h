...
class RtTask {
 public:
  ...
  void print_status(void) {
    if (!is_running_) return;
    printf("|%-20s|%6.4f|%6.4f|%6.4f\n", name_.c_str(), max_runtime_, period_s_,
           max_period_);
  }
 ...
 private:
  ...
  void loop_function(void) {
    ...
    bool is_loop = ((seconds > 0) || (nanoseconds > 10000))
                       ? true
                       : false;  // 如果时间小于10us只运行一次func
    do {
...
class RtPeriodicFun : public RtTask {
 public:
  RtPeriodicFun(float period_s, std::string name, long stack_size, int priority,
                void (*function)())
...
class RtPeriodicMemberFun : public RtTask {
 public:
  RtPeriodicMemberFun(float period_s, std::string name, long stack_size,
                      int priority, void (T::*function)(), T* obj)
      : RtTask(period_s, name, stack_size, priority),
        _function(function),
        _obj(obj) {}
